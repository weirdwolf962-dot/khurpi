...
with st.expander("📸 Upload leaf photo for diagnosis", expanded=False):
    ...
    with plant_col:
        plant_options = ["Select plant..."] + sorted(list(PLANT_COMMON_DISEASES.keys()))
        sel_plant = st.selectbox("Your plant", plant_options, label_visibility="collapsed", key="plant_sel")
        if sel_plant == "Select plant...":
            custom_plant = st.text_input("Or type plant name", placeholder="e.g. Banana, Mango, Wheat...", label_visibility="collapsed", key="custom_plant")
            chosen_plant = custom_plant.strip() if custom_plant and custom_plant.strip() else None
        else:
            chosen_plant = sel_plant

    with region_col:
        region = st.selectbox("Region", REGIONS, label_visibility="collapsed", key="region_sel")
        soil = st.selectbox("Soil type", SOIL_TYPES, label_visibility="collapsed", key="soil_sel")
    ...
    if uploaded and chosen_plant:
        imgs = [Image.open(f) for f in uploaded[:3]]
        img_cols = st.columns(len(imgs))
        for col, img in zip(img_cols, imgs):
            with col: st.image(resize_image(img.copy()), use_container_width=True)

        if st.button("🔍 Diagnose Now", use_container_width=True, key="diag_btn"):
            with st.spinner("Analysing your plant..."):
                try:
                    common = PLANT_COMMON_DISEASES.get(chosen_plant, "various plant diseases")
                    prompt = f"""{lang_instruction}

You are an expert plant pathologist specialising in {chosen_plant} diseases.
Common diseases: {common}
  ...
  "treat_reason": "One sentence plain-language reason"
                    ...
                    if result:
                        ...
                        st.session_state.messages.append({
                            "role": "user",
                            "content": f"📸 Uploaded {len(imgs)} photo(s) of {chosen_plant} from {region}"
                        })
    ...
    elif uploaded and not chosen_plant:
        st.warning("Please select your plant first.")
